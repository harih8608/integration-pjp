	...
	$.ajax('menulist.json', 
{
    ...
    success: function (data,status,xhr) {   
			...
			{
				//submenuaccordwrap="";
				//submenuaccordwrap = submenuaccordwrap + '';
				...
				for(i = 0; i < datamitems.submenu1.length; i++) {
					...
					submenuaccord = submenuaccord+ '<a href="'+datamitems.submenu1[i].href+'" class="dropdown-item" title="'+datamitems.submenu1[i].title+'">'+datamitems.submenu1[i].type+'</a>';
				}
				menuaccord = menuaccord + submenuaccord + '</div></div>';
				submenuaccord = "";
			}
			else
			{
				menuaccord = menuaccord + '<a href="'+datamitems.href+'" class="nav-item nav-link font-weight-bold" title="'+datamitems.title+'">'+datamitems.type+'</a>';
			}
...
accorhtml = accorhtml + "<div class=\"accordion\"><div class=\"acctool\" style=\"font-size:14px;background-color:#82298c;\">" + choiceOption + "&nbsp;(<strong class=\"accvers\" style=\"color:white;font-weight:bold;\">"+seggregate(jmespath.search(majorVersions,"[].minorVersions[?colourCode=='GREEN'].minorVersion | []"))	+"</strong>)<span class=\"lastupd\" style=\"float:right;height:auto;color:white;font-weight:bold;\"> Last update on: " + dateymd_to_dmy(currentTool[0].infoAsOn) +"</span></div><div class=\"card-body\" style=\"overflow-wrap: break-word;background-color:#f2e6ff;\"><p><div style=\"display: block;float: right;\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"javascript:void(0)\" onclick=\"window.open(\'\',\'_blank\').location.href=\'https://ossrepository.gov.in/toollister/file-list.php?tool="+encodevalue(choiceOption)+"&tool_id="+encodevalue(currentTool[0].toolId)+"&user_role="+encodevalue(roleid)+"&user_id="+encodevalue(userid)+"\';return false;\" target=\"_blank\"><img src=\"assets/icons_round/download.png\" style=\"height:25px;\" title=\"download\" ></img></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<div class=\"et\" style=\"\">&nbsp;&nbsp;<img src=\"images/advisory6.png\" style=\"height:25px;\" ></img>&nbsp;&nbsp;<div class=\"et-left et-arrow\" id=\"ttcontent\" style=\" font-size:12px;position:absolute;z-index:10;\"><i>Advisory</i> <br/> Upgrade : "+advisory+"</div></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div></p>"
...
var advisory = JSON.stringify( jmespath.search(majorVersions,"[0].advisory.update[0]"), null,4);
//console.log("Advisory :"+ JSON.stringify(advisory,null,4));
accorhtml = accorhtml + "<div class=\"\"><div class=\"acctool\" style=\"font-size:14px; color:white;background-color:#82298c;\">" + choiceOption + "&nbsp;(<strong class=\"accvers\" style=\"color:white;\">"+seggregate(jmespath.search(majorVersions,"[].minorVersions[?colourCode=='GREEN'].minorVersion | []"))	+"</strong>)<span class=\"lastupd\" style=\"float:right;height:auto;color:white;\"> Last update on: " + dateymd_to_dmy(currentTool[0].infoAsOn) +"</span></div><div class=\"card-body\" style=\"overflow-wrap: break-word;background-color:#f2e6ff;\">"
...
{
	var colornotes = "<div class=\"clearfix\" style=\"color:#808080;\"><div class=\"colornote float-right\"><ul><li><div class=\"input-color\"><span class=\"textgreen\">Recommended</span><div class=\"color-box green\"></div></div></li><li><div class=\"input-color\"><span class=\"textamber\">Still Supported</span><div class=\"color-box amber\"></div></div></li><li><div class=\"input-color\"><span class=\"textred\">NOT Supported</span><div class=\"color-box red\"></div></div></li></ul></div></div>";
	return colornotes;
}
		...
		for(b=0; b < minors.length; b++) {
			if( minors[b].minorVersion == minorversionselected )
			{
				...
				if((minors[b].colourCode).trim() == "AMBER" )
				{
					...
					$("#versioncheckeditem").html("<b>"+$("#versioncheckitem").val()+" version "+ $("#SelectVersionNo").find(":selected").text() +" is still supported </b><br/><br/>However you are advised to upgrade/update to <b style=\"color:#00994c;\">Version "+advisory+"</b><b style=\"color:#375e97;\">*</b><br/><br/><b style=\"color:#375e97;background-color:yellow;\">*While upgrading/updating, you may ensure that the application is tested on staging server before deployment on production. <i style=\"color:#375e97;\">(Metadata updated as on "+ dateymd_to_dmy(currentTool[0].infoAsOn)+")</i></b>");
					$("#versioncheckeditem").css('color','#375e97');
					
				}
				else if ((minors[b].colourCode).trim() == "GREEN")
				{
					$("#versioncheckeditem").html("<b>"+$("#versioncheckitem").val()+" version "+ $("#SelectVersionNo").find(":selected").text() +" is recommended. </b>");
					$("#versioncheckeditem").css('color','#00994c');
		...
		for(b=0; b < minors.length; b++) {
			if( minors[b].minorVersion == minorversionselected )
			{
				...
				if((minors[b].colourCode).trim() == "AMBER" )
				{
					advisory ="";
					advisory = getcolorforversions_upgrade( seggregatebrac_comma(JSON.stringify( jmespath.search(majorVersions,"[0].advisory.update"), null,4)))+"</span>";
					$("#versioncheckeditem_card").html("<b>"+$("#versioncheckitem_card").val()+" version "+ $("#SelectVersionNo_card").find(":selected").text() +" is still supported </b><br/><br/>However you are advised to upgrade/update to <b style=\"color:#00994c;\">Version "+advisory+"</b><b style=\"color:#375e97;\">*</b><br/><br/><b style=\"color:#375e97;background-color:yellow;\">*While upgrading/updating, you may ensure that the application is tested on staging server before deployment on production.  <i style=\"color:#375e97;\">(Metadata updated as on "+ dateymd_to_dmy(currentTool[0].infoAsOn)+")</i></b>");
					$("#versioncheckeditem_card").css('color','#375e97');
					
				}
				else if ((minors[b].colourCode).trim() == "GREEN")
				{
					$("#versioncheckeditem_card").html("<b>"+$("#versioncheckitem_card").val()+" version "+ $("#SelectVersionNo_card").find(":selected").text() +" is recommended. </b>");
