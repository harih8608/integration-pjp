	...
	for (a=0; a < majorVersions.length; a++)
	{
		...
		for(b=0; b < minors.length; b++) {
			if( minors[b].minorVersion == minorversionselected )
			{
				...
				var advisory_update =  jmespath.search(majorVersions,"[].minorVersions[?colourCode == 'PaleGreen'].minorVersion");
				advisory_update = manipulate_minorverscolor(advisory_update);
				//advisory_upgrade ="";
				...
				if((minors[b].colourCode).trim() == "LightSkyBlue" )
				{
					...
					var advisory =  jmespath.search(majorVersions,"[].minorVersions[?colourCode == 'PaleGreen'].minorVersion");
					advisory = manipulate_minorverscolor(advisory);
	...
	for (a=0; a < majorVersions.length; a++)
	{
		...
		for(b=0; b < minors.length; b++) {
			if( minors[b].minorVersion == minorversionselected )
			{
				...
				//advisory_update ="";
				//advisory_update = getcolorforversions_upgrade( seggregatebrac_comma(JSON.stringify( jmespath.search(majorVersions,"["+a+"].advisory.update"), null,4)));
				//advisory_update = getcolorforversions_upgrade( seggregatebrac_comma(JSON.stringify( jmespath.search(majorVersions,"[].minorVersions[?colourCode == 'PaleGreen'].minorVersion"), null,4)));
				//interim= advisory_update+"</span>";
				var advisory_update =  jmespath.search(majorVersions,"[].minorVersions[?colourCode == 'PaleGreen'].minorVersion");
				advisory_update = manipulate_minorverscolor(advisory_update);
				...
				if((minors[b].colourCode).trim() == "LightSkyBlue" )
				{
					advisory ="";
					//advisory = getcolorforversions_upgrade( seggregatebrac_comma(JSON.stringify( jmespath.search(majorVersions,"[0].advisory.update"), null,4)))+"</span>";
					//advisory = jmespath.search(majorVersions,"[].minorVersions[?colourCode == 'PaleGreen'].minorVersion")+"</span>";
					var advisory =  jmespath.search(majorVersions,"[].minorVersions[?colourCode == 'PaleGreen'].minorVersion");
					advisory = manipulate_minorverscolor(advisory);
...
{
	var currentTool = jmespath.search(data, "ossTools[?functionalDomain=='" + $.trim(domainname) + "']");
...
function manipulate_minorverscolor(advisory)
{
	var arr = String(advisory).split(',');
	var latestver="";
	...
	return latestver.substring(0, latestver.length-2);
}