...
if ( toolNameList.length > 0 ){
	...
	if ( accorhtml != "" )
		accorhtml = accorhtml + "<div class=\"accordion\"><div onclick=\"ShowTools(this,\'"+data+"\')\" style=\"font-size:14px;\">" + functionalDomainCurrent + "</div><div >"
	else 
		accorhtml = "<div class=\"accordion\"><div onclick=\"ShowTools(this,\'"+data+"\')\" style=\"font-size:14px;\">" + functionalDomainCurrent + "</div><div>"

}
//console.log(toolNameList);
accorhtml = accorhtml + "<div class=\"tools\"></div></div></div>"
break;
} // end of end domain
//console.log(accorhtml);
return(accorhtml);
...
function ShowTools(element, data)
{
	var toolsaccorhtml="";
	//alert(element.textContent);
	var functionalDomainCurrent = element.textContent;
	...
	if ( toolNameList.length > 0 ){
		...
		for ( i in toolNameList ){
			...
			var currentTool = jmespath.search(data, "ossTools[?toolName=='" + toolNameList[i] + "']");
			//var colourCodes = ["GREEN","AMBER","RED" ] ;
			var colourCodes = ["GREEN","AMBER" ] ;
			//console.log("currentTool-------------------" + JSON.stringify(currentTool,null,4));
			//console.log("currentTool-------------------" + currentTool[0].infoAsOn);
			...
			var advisory = JSON.stringify( jmespath.search(majorVersions,"[0].advisory.update[0]"), null,4);
			//console.log("Advisory :"+ JSON.stringify(advisory,null,4));
			toolsaccorhtml = toolsaccorhtml + "<div class=\"accordion\" style=\"font-size:14px;\">" + toolNameList[i] + "&nbsp;(<strong style=\"color:\#00994C;\">"+seggregate(jmespath.search(majorVersions,"[].minorVersions[?colourCode=='GREEN'].minorVersion | []"))	+"</strong>)<span style=\"float:right;height:auto;\"> Last update on: " + dateymd_to_dmy(currentTool[0].infoAsOn) +"</span></div><div class=\"card-body\" style=\"overflow-wrap: break-word;\"><p><div class=\"tooldetails\"></div></p></div>"
			}
	}
	//console.log(toolNameList);
	//toolsaccorhtml = toolsaccorhtml + "</div></div>"
	
	
	//$(element).next('div > div').html(disaccordion(toolsaccorhtml));
	$(element).next('div').append(toolsaccorhtml);
$(".accordion").accordion();
//$(".accordion").collapse('toggle');
	
}

function disaccordion(htmltext) {

$("#accToolList").append(htmltext);
$(".accordion").accordion();
$(".accordion").collapse('toggle');
